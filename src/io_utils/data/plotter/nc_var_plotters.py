...
import numpy as np
from io_utils.data.write.reg_nc_image import ReadNcImg


class NcVarPlotter(ReadNcImg):
    """
    Class to plotter variables from a netcdf file.

    Parameters
    ----------
    filepath : str
        Path to a file that contains the data to plotter
    ...
    resxy : tuple or None
        ...
        make scatter plotter maps instead of regular gridded maps (as for ISMN stations).
    ...
    fill_values: dict, optional (default: None)
        Names and one or multiple values that should be nan in the plotter
    """
...
class NcVarCombPlotter(object):
    ...
    def plot_comb_variable(self, vards1, vards2, time1=None, time2=None,
        ...
        transparent: bool, optional (default: True)
            Make the background of the plotter transparent.
        plotfile_name : str, optional (default: None)
            File name to store the plotter, if this is None, we create a file name.
        file_format : str, optional (default: 'png')
            Format in which the plotter is stored (e.g. pdf or svg for vector format)
